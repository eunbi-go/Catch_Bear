...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_C_USE_SILENCE] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::C_USE_SILENCE>(Handle_C_USE_SILENCE, session, buffer, len); };
		...
		GPacketHandler[PKT_C_RESTART] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::C_RESTART>(Handle_C_RESTART, session, buffer, len); };
	}
	...
	static SendBufferRef MakeSendBuffer(Protocol::S_USE_SILENCE& pkt) { return MakeSendBuffer(pkt, PKT_S_USE_SILENCE); }
	...
	static SendBufferRef MakeSendBuffer(Protocol::S_RESTART& pkt) { return MakeSendBuffer(pkt, PKT_S_RESTART); }
