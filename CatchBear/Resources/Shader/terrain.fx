...
PatchTess ConstantHS(InputPatch<VS_OUT, 3> input, int patchID : SV_PrimitiveID)
{
    ...
    float minDistance = g_vec2_1.x;     // 테셀레이션이 적용되는 최소 거리
    float maxDistance = g_vec2_1.y;     // 테셀레이션이 적용되는 최대 거리

    // 삼각형 변마다 두 점의 평균을 구해서 점 3개를 구함
    ...
    // 월드좌표계로 옮긴 후 카메라와의 거리에 따라 레벨을 반환해주는 함수
...
// [Terrain Shader]
// g_int_1      : TileX     타일 사이즈
// g_int_2      : TileZ
// g_float_0    : Max Tessellation Level  
// g_vec2_0     : HeightMap Resolution      하이트맵 해상도
